...
class ImageLunarLanderEnv:
    ...
    """
    def __init__(self, env_id='LunarLander-v3', num_frames=4, img_size=(256, 256)):
        """
        初始化环境和处理参数。
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=255, 
            shape=(self.num_frames, self.img_size[0], self.img_size[1]),  #4，256，256
            dtype=np.uint8
        )
    ...
    def _get_stacked_frames(self):
        ...
        stacked_state=np.stack(list(self.frame_stack), axis=0)
        #print("stacked_state:",stacked_state)
    ...
    def step(self, action):
        ...
        Returns:
            tuple: (next_stacked_state, reward, terminated, truncated, info)
                   next_stacked_state (np.ndarray): 下一A个堆叠帧状态。
                   reward (float): 奖励。
